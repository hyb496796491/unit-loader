...
function getTransformPlugin(pluginOptions){
    ...
    if(typeof use !== 'function') throw new Error('the use in plugin option must be function or function name');
    return function(...arg){
                ...
                rule.walk((node)=>{
                    if(node.type === 'decl'){
                        ...
                        if(isNeedToTransform(prop,include,exclude)){
                            ...
                            const valueAfterTransform = transformPlugin.call(null,value,prop,selector);
                            if(typeof valueAfterTransform !== 'string') throw new Error('the transform plugin must return string!');
